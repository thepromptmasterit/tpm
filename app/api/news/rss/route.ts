...
export async function GET() {
  try {
    ...
    // Crea il feed RSS
    const feed = new RSS({
      title: "FutureTools News - Ultime notizie sull'AI",
      description: "Le ultime notizie sull'intelligenza artificiale tradotte in italiano",
      feed_url: "https://tpm.vercel.app/api/news/rss",
      site_url: "https://tpm.vercel.app",
      language: "it",
      pubDate: new Date().toUTCString(),
      ttl: 60, // Aggiornamento ogni ora
    })

    // Aggiungi gli elementi al feed
    translatedNewsItems.forEach(item => {
      const newsDate = parseDate(item.date)
      feed.item({
        title: item.title,
